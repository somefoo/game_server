...
class global_player_state{
  ...
  void update(const player_action* const a){
    assert(a != nullptr);
    assert(a->m_player_id < MAX_PLAYER_COUNT);
  ...
  void set_data(const player_runtime_state<MAX_PLAYER_COUNT>* const runtime_state_data){
    std::memcpy(&m_runtime_state.m_player_runtime_data, runtime_state_data, sizeof(const player_runtime_state<MAX_PLAYER_COUNT>));
  }

  void set_packet(const packet<const player_runtime_state<MAX_PLAYER_COUNT>>* const runtime_state_packet){
    std::memcpy(&m_runtime_state, runtime_state_packet, sizeof(packet<const player_runtime_state<MAX_PLAYER_COUNT>>));
  }

  //TODO Change name; remove runtime
  constexpr std::pair<const packet<player_runtime_state<MAX_PLAYER_COUNT>>*, size_t> get_state_packet(void) const{
    //TODO This needs to be adaptive later!
    return std::make_pair(&m_runtime_state_packet, sizeof(packet<player_runtime_state<MAX_PLAYER_COUNT>>)); 
  }

  constexpr const player_runtime_state<MAX_PLAYER_COUNT>* get_state(void) const{
    //return nullptr;
    return &(m_runtime_state); 
  }

  constexpr uint8_t add_player(void){
    assert(m_current_count < MAX_PLAYER_COUNT - 1);
  ...
  packet<player_runtime_state<MAX_PLAYER_COUNT>> m_runtime_state_packet;
  player_runtime_state<MAX_PLAYER_COUNT> &m_runtime_state = m_runtime_state_packet.m_packet;
